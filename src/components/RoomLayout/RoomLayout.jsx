...
function drawDownlight(c, x, y, on, br, lr, lg, lb) {
  ...
  c.strokeStyle = on?`rgba(${lr},${lg},${lb},0.90)`:'rgba(255,255,255,0.10)';
  c.lineWidth = 1;
  // c.beginPath(); c.arc(x,y,11,0,Math.PI*2); c.stroke();
  // c.fillStyle = on?'#06080e':'rgba(0,0,0,0.55)';
...
export default function RoomLayout() {
  ...
  const draw = useCallback(() => {
    ...
    const WALL = 14;
    const WX=WALL, WY=WALL, WW=W-WALL*2, WH=H-WALL*2;



    /* ── floor / background image ── */
    c.save();
    rr(c,WX,WY,WW,WH,3); c.clip();
    ...
    if (bgReadyRef.current && bgImageRef.current) {
      ...
      const vig=c.createRadialGradient(WX+WW/2,WY+WH/2,WH*0.2,WX+WW/2,WY+WH/2,WH*0.75);
      // vig.addColorStop(0,'rgba(0,0,0,0)'); vig.addColorStop(1,'rgba(0,0,0,0.20)');
      c.fillStyle=vig; c.fillRect(WX,WY,WW,WH);
    } else {
    ...
    function ledStrip(side) {
      ...
      if (br>0){
        ...
        const step=9; const cnt=Math.floor(len/step);
        for(let i=0;i<cnt;i++){
          const t=i*step+4;
    ...
    valG.addColorStop(0, '#2a2018');
    valG.addColorStop(1, '#1a1410');
    c.fillStyle = valG;
    c.fillRect(CX-5, CY2+7, CW+12, 10);
    c.strokeStyle = 'rgba(255,255,255,0.06)'; c.lineWidth = 0.2;
    c.strokeRect(CX-5, CY2+7, CW+12, 10);

    /* track rod */
    c.fillStyle = '#0a0e18';
    c.fillRect(CX-6, CY2+8, CW+14, 3);
    c.strokeStyle = 'rgba(120,160,220,0.18)'; c.lineWidth = 0.4;
    c.strokeRect(CX-6, CY2+8, CW+14, 3);
    ...
    /* fabric panels — hanging ABOVE the rod */
    ...
    if (cpW > 3) {
      ['l','r'].forEach(side => {
        ...
        if (side==='l'){
          fabricG.addColorStop(0.00,'#5a4530');
          fabricG.addColorStop(0.15,'#4a3a28');
          fabricG.addColorStop(0.40,'#3e3020');
          fabricG.addColorStop(0.70,'#362a1c');
          fabricG.addColorStop(0.88,'#3e3020');
          fabricG.addColorStop(1.00,'#2a2014');
        } else {
          fabricG.addColorStop(0.00,'#2a2014');
          fabricG.addColorStop(0.12,'#3e3020');
          fabricG.addColorStop(0.30,'#362a1c');
          fabricG.addColorStop(0.60,'#3e3020');
          fabricG.addColorStop(0.85,'#4a3a28');
          fabricG.addColorStop(1.00,'#5a4530');
        }
        c.fillStyle=fabricG;
        rr(c,cx2,fabricY,cpW,CH,5); c.fill();
        ...
        for (let fi=0; fi<foldCount; fi++){
          ...
          shadowG.addColorStop(0,'rgba(0,0,0,0.36)');
          ...
          hlG.addColorStop(0.6,'rgba(255,255,255,0.055)');
          hlG.addColorStop(1,'rgba(255,255,255,0.02)');
          ...
          if (fi>0){
            ...
            c.beginPath(); c.moveTo(fx+1,fabricY+2); c.lineTo(fx+1,fabricY+CH-2); c.stroke();
          }
        }
        ...
        /* bottom hem */
        // c.strokeStyle='rgba(180,150,100,0.20)'; c.lineWidth=1;
        // c.beginPath(); c.moveTo(cx2+4,fabricY+CH-1); c.lineTo(cx2+cpW-4,fabricY+CH-1); c.stroke();
      });

      /* light bleed through gap */
      if (cO > 0.05) {
    ...
    [60,180,300].forEach(a=>{
      ...
      bG.addColorStop(1,D.fan.speed>0?'rgba(255,255,255,0.12)':'rgba(255,255,255,0.12)');
    ...
    hubG.addColorStop(0,D.fan.speed>0?'rgba(255,255,255,0.95)':'rgba(255,255,255,0.95)');
    hubG.addColorStop(1,D.fan.speed>0?'rgba(255,255,255,0.10)':'rgba(255,255,255,0.10)');
    c.fillStyle=hubG;
    c.strokeStyle=D.fan.speed>0?'rgba(255,255,255,0.40)':'rgba(255,255,255,0.40)'; c.lineWidth=1.1;
    c.beginPath(); c.arc(FCX,FCY,10,0,Math.PI*2); c.fill(); c.stroke();
    c.fillStyle=D.fan.speed>0?'rgba(255,255,255,0.55)':'rgba(255,255,255,0.55)';
  ...
  return (
    <div
      ...
      ref={containerRef}
      style={{width:'600px',height:'400px',borderRadius:'1px',overflow:'hidden'}}
    >
      <canvas
        ref={canvasRef}
        // onClick={handleClick}
        style={{display:'block',width:'600px',height:'400px',cursor:'pointer'}}
      />
    </div>
  );
}